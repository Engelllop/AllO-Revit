...
/// <summary>
/// Implementacion completa de IRevitService para Revit 2025.
/// Identica a Revit 2024 (ambas usan ElementId.Value long).
/// </summary>
public class RevitService : IRevitService
{
        ...
        return new FilteredElementCollector(Doc)
            ...
            .Select(sheet =>
            {
                ...
                var tbInst = new FilteredElementCollector(Doc, sheet.Id)
                    ...
                    .FirstOrDefault();
                if (tbInst != null) { tbName = tbInst.Name; tbTypeId = (int)tbInst.GetTypeId().Value; }
    ...
    public List<ViewInfo> GetAllViews()
    {
        ...
        return new FilteredElementCollector(Doc)
            ...
            .Select(v =>
            {
                ...
                string scale = "";
                try { if (v.Scale > 0) scale = "1:" + v.Scale.ToString(); } catch { }

                string templateName = "";
                try { if (v.ViewTemplateId != ElementId.InvalidElementId) { var tmpl = Doc.GetElement(v.ViewTemplateId); if (tmpl != null) templateName = tmpl.Name; } } catch { }
    ...
    {
        if (Doc == null) return 0;
        using var tx = new Transaction(Doc, "AllO - Toggle Revision Status");
        tx.Start();
        try
        {
            var rev = Doc.GetElement(new ElementId((long)elementId)) as Revision;
    ...
    public List<DisconnectedConnectorInfo> FindDisconnectedElements()
    {
        ...
        foreach (var el in collector)
        {
            ...
            if (disconnected > 0)
            {
                ...
                {
                    ElementId = (int)el.Id.Value, ElementName = el.Name ?? "Unknown", Category = category,
    ...
    private View? CreateLegendView()
    {
        try
        {
            // Try reflection for ViewLegend.Create (may not be available in all Revit versions)
